--[[ Apex Lite+ v2 ]] return(function()
    ...
    local themes = {
        ...
        ["Ocean"] = {bg = Color3.fromRGB(0, 20, 40), frame = Color3.fromRGB(0, 60, 80), btn = Color3.fromRGB(0, 100, 120), text = Color3.fromRGB(150, 230, 255)},
        ["Summer"] = {bg = Color3.fromRGB(60, 40, 20), frame = Color3.fromRGB(100, 70, 30), btn = Color3.fromRGB(140, 100, 50), text = Color3.fromRGB(255, 230, 150)}
    }
    ...
    -- Перетаскивание кнопки (уже есть, оставляем)
    ...
    -- Меню (скруглённое, с полосой для перетаскивания)
    ...
    cornerDragBar.CornerRadius = UDim.new(0, 0, 0, 12) -- Скругление только снизу
    ...
    commandsContent.CanvasSize = UDim2.new(0, 0, 0, 600)
    ...
    local speedGlitchBtn = createCommandBtn("💨 Speed Glitch: OFF", 290, function()
        ...
        if speedGlitchEnabled then
            speedGlitchConnection = game:GetService("RunService").Heartbeat:Connect(function()
                if speedGlitchEnabled and hum and hum:GetState() == Enum.HumanoidStateType.Jumping then
                    hum.WalkSpeed = hum.WalkSpeed * 1.05 -- плавное ускорение в прыжке
                end
            end)
    ...
    -- ===== AIMBOT (наведение, без выстрела) =====
    local aimBtn = createCommandBtn("🎯 Aimbot (наведение): OFF", 330, function()
