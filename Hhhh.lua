--[[
    Project: San Diego Border RP Script (Ultimate Version with AntiTp Bypass)
    Features: Speed, X-Ray, Aimbot, Safe Teleport, Dynamic Player Support
]]
...
-- ==================== БЕЗОПАСНЫЙ ТЕЛЕПОРТ (ОБХОД AntiTp) ====================
local function safeTeleport(targetCFrame)
    ...
    local safeSpeed = 160
    local travelTime = math.max(distance / safeSpeed, 0.3)
    ...
    task.delay(travelTime + 0.5, function()
        ...
        if (HumanoidRootPart.Position - targetPos).Magnitude > 50 then
            HumanoidRootPart.CFrame = targetCFrame
            print("🔄 Повторный телепорт (обход AntiTp)")
        end
        if Humanoid then
    ...
    print("🔄 Телепорт: " .. math.floor(distance) .. " студий за " .. string.format("%.1f", travelTime) .. " сек")
end
