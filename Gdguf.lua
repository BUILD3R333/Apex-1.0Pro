-- =====================================================
-- APEX V7 - FULL PACKAGE FOR ROLEPLAY GAMES (FIXED)
-- =====================================================
...
local Settings = {
    XRayMode = 1,           -- 1=Только розыск, 2=Все, 3=Розыск(красный)/Мирные(синий)
    AimbotMode = 1,         -- 1=Все, 2=Только розыск, 3=Мирные
    AimbotSmooth = 0.12,
    FlySpeed = 50,
    WalkSpeedVal = 50,      -- Теперь можно ставить ЛЮБУЮ скорость
}
...
local function createSlider(parent, text, y, min, max, step, default, callback)
    -- Убираем ограничение max, теперь можно ставить любую скорость
    ...
    btn.MouseButton1Click:Connect(function()
        val = val + step
        -- Убираем верхнее ограничение! (было if val > max then val = min end)
...
local function getColorsForPlayer(p)
    if xrayMode == 1 then
        if isWanted(p) then
            return Color3.fromRGB(255, 0, 0), 0.2
        else
            return nil, nil
        end
    elseif xrayMode == 2 then
        if isWanted(p) then
            ...
            return Color3.fromRGB(0, 200, 255), 0.2
        end
    elseif xrayMode == 3 then
        if isWanted(p) then
            return Color3.fromRGB(255, 0, 0), 0.2
        else
            return Color3.fromRGB(0, 100, 255), 0.15
        end
    end
    return nil, nil
end
...
-- =====================================================
--  ВКЛАДКА MOVEMENT (ИСПРАВЛЕННАЯ СКОРОСТЬ)
-- =====================================================
...
createSlider(movTab, "Скорость полёта", yPos, 20, 150, 5, flySpeed, function(val)
    flySpeed = val
end)
yPos = yPos + 40
...
-- ===== ИСПРАВЛЕННАЯ WALKSPEED (ПРЯМОЕ ПРИМЕНЕНИЕ, БЕЗ ЗАЩИТЫ) =====
...
local function applySpeed(char)
    ...
    if humanoid and speedActive then
        humanoid.WalkSpeed = speedVal -- Прямое применение, никаких BodyVelocity!
...
-- Слайдер без ограничений
local lbl = createLabel(movTab, "Скорость ходьбы: " .. speedVal, yPos)
local btn = Instance.new("TextButton")
btn.Size = UDim2.new(0.3, 0, 0, 25)
btn.Position = UDim2.new(0.65, 0, 0, yPos)
btn.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
btn.Text = tostring(speedVal)
btn.TextColor3 = Color3.fromRGB(255, 255, 255)
btn.TextScaled = true
btn.Font = Enum.Font.GothamBold
btn.Parent = movTab
...
btn.MouseButton1Click:Connect(function()
    speedVal = speedVal + 5 -- Увеличиваем на 5
    btn.Text = tostring(speedVal)
    lbl.Text = "Скорость ходьбы: " .. speedVal
...
-- ===== ЗАЩИТА ДЛЯ FLY/NOCLIP (НЕ ТРОГАЕМ SPEED) =====
...
print("✅ APEX V7 FIXED ЗАГРУЖЕН! Speed без ограничений.")
