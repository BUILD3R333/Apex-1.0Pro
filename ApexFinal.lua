...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.Tab then ApexGui.Enabled = not ApexGui.Enabled end
end)
...
-- ==========================================
-- 🚀 ApexZen V7+ — ЧАСТЬ 2/3
-- ==========================================

-- Функция применения графики
local function applyGraphics(level)
    local shadowTransparency = 0.8; local cornerRadius = 0; local bgTransparency = 0.05; local btnTransparency = 0
    if level == "Низкая" then shadowTransparency = 1; cornerRadius = 0; bgTransparency = 0.2
    elseif level == "Средняя" then shadowTransparency = 0.7; cornerRadius = 8; bgTransparency = 0.1
    elseif level == "Высокая" then shadowTransparency = 0.5; cornerRadius = 12; bgTransparency = 0.05
    else shadowTransparency = 0.4; cornerRadius = 16; bgTransparency = 0.03 end
    MainWindow.BackgroundTransparency = bgTransparency
    Sidebar.BackgroundTransparency = bgTransparency + 0.1
end

local function saveConfig()
    pcall(function() if writefile then writefile("ApexZen_Config.json", HttpService:JSONEncode(states)) end end)
end
...
LocalPlayer.Idled:Connect(function()
    if states.AntiAFK then VirtualUser:CaptureController(); VirtualUser:ClickButton2(Vector2.new()) end
end)
        ...
        if hrp and LocalPlayer.Character then
            ...
            if myHrp then
                ...
                if btnInstance then
                    ...
                    task.delay(0.5, function() if btnInstance then btnInstance.Text = oldText end end)
                end
            end
        end
    end
...
local function collectCoins()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        ...
        if obj:IsA("BasePart") then
            ...
            elseif string.lower(obj.Name):find("coin") then isCoin = true end
        end
        if isCoin then
