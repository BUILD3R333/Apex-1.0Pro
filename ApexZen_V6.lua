-- ==========================================
-- 🚀 ApexZen.lua V6 (ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ)
-- ==========================================
...
MainWindow.Size = UDim2.new(0, 400, 0, 460); MainWindow.Position = UDim2.new(0.5, -200, 0.5, -230)
...
local themes = {
    ...
    ["Уникал: Белая"] = {bg = Color3.fromRGB(240,240,245), side = Color3.fromRGB(220,220,225), btn = Color3.fromRGB(200,200,205), text = Color3.fromRGB(30,30,35)},
    ["Уникал: Летняя"] = {bg = Color3.fromRGB(255,180,50), side = Color3.fromRGB(255,140,0), btn = Color3.fromRGB(255,100,0), text = Color3.fromRGB(255,255,255)},
    ["Уникал: Новогодняя"] = {bg = Color3.fromRGB(30,100,30), side = Color3.fromRGB(180,30,30), btn = Color3.fromRGB(50,150,50), text = Color3.fromRGB(255,255,200)},
}
...
-- Aimbot (оригинальный от Gemini, проверенный)
...
-- 🔹 ИСПРАВЛЕННЫЙ АВТОФАРМ (С NOCLIP И БЕЗОПАСНЫМ ТЕЛЕПОРТОМ)
local function collectCoins()
    ...
    -- Временно включаем Noclip для безопасности
    local noclipWasOn = states.Noclip
    if not noclipWasOn then toggleNoclip(true) end
    ...
    -- Возвращаем Noclip в исходное состояние
    if not noclipWasOn then toggleNoclip(false) end
end
...
-- 🔹 FLY (ОРИГИНАЛЬНЫЙ ОТ GEMINI — РАБОЧИЙ)
local flyBV, flyBG
...
-- 🔹 ОБНОВЛЕНИЕ ЯЗЫКА (для всех кнопок)
