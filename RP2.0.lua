-- =====================================================
-- APEX V7 - FULL PACKAGE FOR ROLEPLAY GAMES
-- =====================================================
...
-- ===== ФУНКЦИЯ ПОИСКА РОЗЫСКА (расширенная) =====
...
local function getColorsForPlayer(p)
    if xrayMode == 1 then -- только розыск
        if isWanted(p) then
            return Color3.fromRGB(255, 0, 0), 0.2
        else
            return nil, nil
        end
    elseif xrayMode == 2 then -- все игроки
        if isWanted(p) then
            return Color3.fromRGB(255, 0, 0), 0.2
        else
            return Color3.fromRGB(0, 200, 255), 0.2
        end
    elseif xrayMode == 3 then -- розыск красный, мирные синий
        if isWanted(p) then
            return Color3.fromRGB(255, 0, 0), 0.2
        else
...
-- Управление Fly (WASD + Space + Shift)
...
createSlider(movTab, "Скорость ходьбы", yPos, 30, 100, 5, speedVal, function(val)
    speedVal = val
    if speedActive then
        applySpeed(false)
        wait(0.1)
...
-- =====================================================
--  ЗАЩИТА ОТ ТЕЛЕПОРТАЦИИ (для Fly и Noclip)
-- =====================================================
...
print("✅ APEX V7 ЗАГРУЖЕН! Все функции активны.")
