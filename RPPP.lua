-- =====================================================
-- APEX V7.1 - SPEED (ТОЛЬКО ПОСЛЕ СМЕРТИ) + X-RAY + СМАЙЛИК
-- =====================================================
...
-- =====================================================
--  ВКЛАДКА SPEED (БАГ - ТОЛЬКО ПОСЛЕ СМЕРТИ)
-- =====================================================
...
createButton(speedTab, "Speed (после смерти)", 75, function(state)
    speedActive = state
...
respawnBtn.Position = UDim2.new(0.3, 0, 0, 115)
...
player.CharacterAdded:Connect(function(newChar)
    char = newChar
    wait(0.5) -- Ждём, пока игра загрузит персонажа
    if speedActive then
        applySpeedOnRespawn(newChar) -- Скорость применяется только здесь!
    end
...
print("✅ APEX V7.1 (БАГ) загружен! Speed работает только после смерти.")
